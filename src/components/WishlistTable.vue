...
import { ArrowUpDown, ChevronDown, Eye, Heart } from "lucide-vue-next";
...
const columns: ColumnDef<WishlistData>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const user = row.original.user;
      return h("div", { class: "text-sm" }, user.email);
    },
  },
  {
  ...
  },
  {
    ...
    header: ({ column }) => {
      return h(
        ...
        () => ["Items Count", h(ArrowUpDown, { class: "ml-2 h-4 w-4" })]
      );
    },
    cell: ({ row }) => {
      const count = row.getValue("wishlistCount") as number;
      return h("div", { class: "flex items-center gap-2" }, [
        h(Heart, { class: "h-4 w-4 text-red-500" }),
        h("span", { class: "font-medium" }, count),
      ]);
    },
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return h(
        Button,
        {
          variant: "ghost",
          size: "sm",
          onClick: () => router.push(`/wishlist/${user._id}`),
        },
        () => [h(Eye, { class: "h-4 w-4 mr-2" }), "View Details"]
      );
    },
  },
];
