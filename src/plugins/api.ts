...
const withAuthHeader = (headers: FetchOptions["headers"], token: string) => {
  ...
  else if (Array.isArray(headers)) headers.forEach(([k, v]) => (h[k] = v as string));
    ...
    install(app: App) {
      ...
      const api = ofetch.create({
        ...
        async onRequest({ options }) {
          ...
          (options as any).headers = withAuthHeader(options.headers, auth.accessToken!);
        },

        async onResponseError({ request, response, options }) {
          const auth = useAuthStore();

          if (response?.status === 401 && !(options as any)[RETRY_KEY] && !(options as any).noAuth) {
